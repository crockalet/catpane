...
pub fn draw_log_area(ui: &mut Ui, pane_id: PaneId, app: &mut App) {
    ...
    let search_highlight_fi = if pane.search_open && !pane.search_match_indices.is_empty() {
        Some(pane.search_match_indices.get(pane.search_current).copied().unwrap_or(0))
    } else {
        None
    };
    ...
    let is_auto_scroll = pane.auto_scroll;

    let mut scroll_area = ScrollArea::vertical()
        .id_salt("log_scroll")
        .auto_shrink([false, false])
        .stick_to_bottom(is_auto_scroll)
        .animated(false);

    if wants_scroll_to_bottom {
        let max_offset = (total_rows as f32) * LOG_ROW_HEIGHT;
    ...
    scroll_area.show_rows(ui, LOG_ROW_HEIGHT, total_rows, |ui, row_range| {
        for fi in row_range {
            if fi >= pane.filtered_indices.len() { break; }
            let ei = pane.filtered_indices[fi];
            if ei >= pane.entries.len() { continue; }
            ...
            let bg_color = if is_search_current {
                Some(if is_dark { Color32::from_rgb(100, 90, 0) } else { Color32::from_rgb(255, 240, 150) })
            } else if is_selected {
                Some(if is_dark { OD_BG_HL } else { OL_BG_HL })
            } else if is_search_match {
                Some(if is_dark { Color32::from_rgb(50, 45, 20) } else { Color32::from_rgb(255, 250, 220) })
            } else {
                None
            };
            ...
            let msg_galley = painter.layout_no_wrap(
                entry.message.clone(),
                egui::FontId::monospace(12.0),
                if is_dark { OD_FG } else { OL_FG },
            );
            painter.galley(text_pos, msg_galley, Color32::TRANSPARENT);

            // Right-click context menu
            let entry_tag = entry.tag.clone();
            let entry_line = format!("{} {} {} {}", entry.timestamp, entry.level.as_char(), entry.tag, entry.message);
            let entry_msg = entry.message.clone();
            row_resp.context_menu(|ui| {
                if let Some((lo, hi)) = pane.selected_range() {
                    if hi > lo {
                        let count = hi - lo + 1;
                        if ui.button(format!("📋 Copy {} selected lines", count)).clicked() {
                            let lines: Vec<String> = (lo..=hi)
                                .filter_map(|f| {
                                    pane.filtered_indices.get(f)
                                        .and_then(|&e| pane.entries.get(e))
                                        .map(|e| format!("{} {} {} {}", e.timestamp, e.level.as_char(), e.tag, e.message))
                                })
                                .collect();
                ...
                if ui.button("📋 Copy line").clicked() {
                    ui.ctx().copy_text(entry_line.clone());
                    ui.close_menu();
                }
                if ui.button("📋 Copy message").clicked() {
                    ui.ctx().copy_text(entry_msg.clone());
                    ui.close_menu();
                }
                ui.separator();
    ...
    {
        let user_scrolled_up = ui.input(|i| {
