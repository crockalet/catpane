---
name: catpane-logcat
description: Use this when you need to inspect Android adb/logcat output or booted iOS simulator logs through CatPane's MCP server, including starting captures, checking status, and querying buffered logs with filters and cursors.
---

# CatPane device-log MCP

Use this skill when you need Android logcat or booted iOS simulator logs from CatPane's MCP runtime. It is for live debugging, reproductions, and incremental monitoring; it is not a generic platform logging tutorial.

## Use it when

- you need to discover whether a useful capture already exists
- you need to start a scoped capture for one device, app, or PID
- you need focused log queries instead of dumping raw logs
- you need to poll for new logs during an investigation

## Tool surface

- `get_status` — inspect captures and optionally include connected Android devices and booted iOS simulators
- `list_devices` — list currently connected Android devices and booted iOS simulators
- `start_capture` — start buffering logs for a device; `package` and `pid` are Android-only
...
2. **Start capture only when needed.**
   ...
   - Use either `package` or `pid`, not both; both are Android-only.
...
3. **Query with focused filters.**
   ...
   - For iOS simulator captures, also use `process`, `subsystem`, and `category` when they help narrow results.
...
- `text` is a case-insensitive substring filter over tag, message, and iOS process/subsystem/category metadata.
- `process`, `subsystem`, and `category` are case-insensitive substring filters for iOS simulator captures.
