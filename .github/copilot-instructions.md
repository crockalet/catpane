...
```sh
cargo run                  # debug build (recommended during dev)
...
The app is split into two logical layers:

**Core (no egui dependency):**
- `src/log_entry.rs` – `LogEntry` struct and `parse_logcat_line()` which parses `logcat -v threadtime` output (format: `MM-DD HH:MM:SS.mmm  PID  TID LEVEL TAG: message`)
- `src/filter.rs` – `Filter` and `TagFilter` (Include/Exclude/Regex); all filtering logic lives here, including vendor noise suppression
- `src/pane.rs` – `Pane` (per-pane state: entries, filtered indices, search, selection) and `PaneNode` (binary split tree of `PaneId`s); max 50,000 entries per pane with compaction
- `src/adb.rs` – all ADB interaction: device listing, logcat spawning (`spawn_logcat`), device tracking (`spawn_device_tracker`), wireless pairing via mDNS, QR code generation

**UI (egui/eframe):**
- `src/main.rs` – `CatPaneApp` implements `eframe::App`; owns the Tokio runtime, handles native menu events (via `muda`), copy logic, and the `window_should_update` visibility check (macOS occlusion)
- `src/app.rs` – `App` struct (pane map + pane tree + device list + session/history persistence); session saved to `~/.config/catpane/session.json`, tag history to `~/.config/catpane/tag_history.txt`
- `src/ui/` – rendering only; `mod.rs` walks the `PaneNode` tree and renders each leaf with `draw_pane_panel`; sub-modules handle toolbar, tag bar, log area, search bar, dialogs

**Data flow:**
`adb::spawn_logcat` → async Tokio task → `mpsc::channel` → `Pane::ingest_lines()` (called each frame) → `Pane::entries` + `Pane::filtered_indices` → egui rendering
...
**Async/sync boundary:** The app runs on a Tokio multi-thread runtime but egui is synchronous. ADB calls that must happen during a frame (device list, package list) use `rt_handle.block_on(...)` in `main.rs`. Background tasks (logcat, device tracker) communicate back via `mpsc` channels read with `try_recv()` each frame.
...
**Auto-fork:** `main()` re-spawns itself as a detached process (with `CATPANE_FORKED=1`) so `cargo run` / terminal launch doesn't keep the terminal attached.
